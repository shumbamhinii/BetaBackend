...
const upload = (0, multer_1.default)({ storage: multer_1.default.memoryStorage() }); // Use memory storage for file uploads
// Utility function to map DB schema to frontend interface
const mapSupplierToFrontend = (supplier) => ({
...
app.get('/transactions', async (req, res) => {
  ...
  `;
    ...
    catch (error) { // Changed 'err' to 'error: unknown'
...
app.post('/transactions/manual', async (req, res) => {
    ...
    try {
        ...
        if (id) {
            ...
            result = await pool.query(`UPDATE transactions
         SET
           "type" = $1,
           amount = $2,
           description = $3,
           ...
           confirmed = $9      // Added
         WHERE id = $10
         RETURNING id, "type", amount, description, "date", category, account_id, created_at, original_text, source, confirmed`, [type, amount, description || null, date, category || null, account_id || null, original_text || null, source || 'manual', confirmed !== undefined ? confirmed : true, id]);
            ...
            result = await pool.query(`INSERT INTO transactions ("type", amount, description, "date", category, account_id, original_text, source, confirmed)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING id, "type", amount, description, "date", category, account_id, created_at, original_text, source, confirmed`, [type, amount, description || null, date, category || null, account_id || null, original_text || null, source || 'manual', confirmed !== undefined ? confirmed : true]);
        }
...
app.post('/accounts', async (req, res) => {
    ...
    try {
        ...
        `INSERT INTO accounts (type, name, code) VALUES ($1, $2, $3) RETURNING id`, [type, name, code]);
        const insertedId = insert.rows[0].id;
        const fullAccount = await pool.query(
        // Select the inserted account, including 'type' and 'code'
        `SELECT id, name, type, code FROM accounts WHERE id = $1`, [insertedId]);
...
app.post('/assets', async (req, res) => {
    const { type, name, number, cost, date_received, account_id, depreciation_method, useful_life_years, salvage_value } = req.body;
    ...
    try {
        const insert = await pool.query(`INSERT INTO assets (
...
app.put('/assets/:id', async (req, res) => {
    const { id } = req.params;
    const { type, name, number, cost, date_received, account_id, depreciation_method, useful_life_years, salvage_value, accumulated_depreciation, last_depreciation_date } = req.body;
    const updates = [];
    const values = [];
    let paramIndex = 1;
    if (type !== undefined) {
    ...
    if (date_received !== undefined) {
        updates.push(`date_received = $${paramIndex++}`);
    ...
    if (depreciation_method !== undefined) {
        ...
        values.push(depreciation_method || null);
    }
    if (useful_life_years !== undefined) {
        updates.push(`useful_life_years = $${paramIndex++}`);
        values.push(useful_life_years || null);
    }
    if (salvage_value !== undefined) {
        updates.push(`salvage_value = $${paramIndex++}`);
        values.push(salvage_value || null);
    }
    if (accumulated_depreciation !== undefined) {
        updates.push(`accumulated_depreciation = $${paramIndex++}`);
        values.push(accumulated_depreciation);
    }
    if (last_depreciation_date !== undefined) {
        updates.push(`last_depreciation_date = $${paramIndex++}`);
        values.push(last_depreciation_date || null);
    }
    if (updates.length === 0) {
    ...
    try {
        const result = await pool.query(`UPDATE assets SET ${updates.join(', ')} WHERE id = $${paramIndex} RETURNING *`, values);
        if (result.rows.length === 0) {
...
const calculateDepreciation = (cost, salvageValue, usefulLifeYears, startDate, endDate) => {
...
app.post('/api/depreciation/run', async (req, res) => {
    ...
    `, [calculationEndDate.toISOString().split('T')[0]]); // Compare only date part
        ...
        const expenseAccountResult = await client.query(`SELECT id FROM accounts WHERE name ILIKE 'Depreciation Expense' OR name ILIKE 'Other Expenses' LIMIT 1`);
        ...
        for (const asset of assetsResult.rows) {
            ...
            if (depreciationStartDate <= effectiveCalculationEndDate) {
                const depreciationAmount = calculateDepreciation(assetCost, assetSalvageValue, assetUsefulLifeYears, depreciationStartDate, effectiveCalculationEndDate);
                if (depreciationAmount > 0) {
                    ...
                    await client.query(`UPDATE assets SET accumulated_depreciation = $1, last_depreciation_date = $2 WHERE id = $3`, [newAccumulatedDepreciation, effectiveCalculationEndDate.toISOString().split('T')[0], asset.id]);
                    // 2. Create a transaction for depreciation expense
                    const transactionResult = await client.query(`INSERT INTO transactions (type, amount, description, date, category, account_id)
             VALUES ($1, $2, $3, $4, $5, $6) RETURNING id`, [
                    ...
                    await client.query(`INSERT INTO depreciation_entries (asset_id, depreciation_date, amount, transaction_id)
             VALUES ($1, $2, $3, $4)`, [asset.id, effectiveCalculationEndDate.toISOString().split('T')[0], depreciationAmount, transactionId]);
...
app.post('/expenses', async (req, res) => {
    ...
    try {
        const insert = await pool.query(`INSERT INTO expenses (name, details, category, amount, date, account_id)
       VALUES ($1, $2, $3, $4, $5, $6) RETURNING id`, 
...
app.put('/expenses/:id', async (req, res) => {
    ...
    values.push(id); // Add ID for WHERE clause
    try {
        const result = await pool.query(`UPDATE expenses SET ${updates.join(', ')}, updated_at = CURRENT_TIMESTAMP WHERE id = $${paramIndex} RETURNING *`, values);
        if (result.rows.length === 0) {
...
app.post('/customers', async (req, res) => {
    ...
    try {
        const result = await pool.query(`INSERT INTO customers (name, contact_person, email, phone, address, tax_id)
       VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`, [name, contact_person || null, email || null, phone || null, address || null, tax_id || null]);
...
app.get('/api/customers/search', async (req, res) => {
    ...
    try {
        const result = await pool.query(`SELECT id, name FROM public.customers WHERE LOWER(name) LIKE $1 ORDER BY name`, [searchTerm]);
...
app.get('/api/customers/:id', async (req, res) => {
    const { id } = req.params;
    try {
        const result = await pool.query('SELECT id, name, contact_person, email, phone, address, tax_id, total_invoiced FROM public.customers WHERE id = $1', [id]);
        if (result.rows.length === 0) {
...
app.post('/api/customers', async (req, res) => {
    const { name, contactPerson, email, phone, address, vatNumber } = req.body;
    if (!name) { // Name is NOT NULL in DB
        return res.status(400).json({ error: 'Customer name is required' });
    }
    try {
        const result = await pool.query(`INSERT INTO public.customers (name, contact_person, email, phone, address, tax_id, total_invoiced)
             VALUES ($1, $2, $3, $4, $5, $6, 0.00) RETURNING id, name, contact_person, email, phone, address, tax_id, total_invoiced`, [name, contactPerson || null, email || null, phone || null, address || null, vatNumber || null]);
...
app.put('/api/customers/:id', async (req, res) => {
    ...
    try {
        const result = await pool.query(`UPDATE public.customers
             SET name = $1, contact_person = $2, email = $3, phone = $4, address = $5, tax_id = $6, updated_at = CURRENT_TIMESTAMP
             WHERE id = $7 RETURNING id, name, contact_person, email, phone, address, tax_id, total_invoiced`, [name, contactPerson || null, email || null, phone || null, address || null, vatNumber || null, id]);
        if (result.rows.length === 0) {
            return res.status(404).json({ error: 'Customer not found.' });
        }
        res.json(mapCustomerToFrontend(result.rows[0]));
...
app.delete('/api/customers/:id', async (req, res) => {
    const { id } = req.params;
    try {
        const { rowCount } = await pool.query('DELETE FROM public.customers WHERE id = $1', [id]);
        if (rowCount === 0) {
            return res.status(404).json({ error: 'Customer not found.' });
        }
...
app.post('/vendors', async (req, res) => {
    ...
    try {
        const result = await pool.query(`INSERT INTO vendors (name, contact_person, email, phone, address, tax_id)
       VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`, [name, contact_person || null, email || null, phone || null, address || null, tax_id || null]);
        res.status(201).json(result.rows[0]);
    }
    catch (error) { // Changed 'err' to 'error: unknown'
...
app.post('/products-services', async (req, res) => {
    ...
    try {
        const result = await pool.query(`INSERT INTO products_services (name, description, unit_price, cost_price, sku, is_service, stock_quantity)
       VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *`, [name, description || null, unit_price, cost_price || null, sku || null, is_service || false, stock_quantity || 0]);
        res.status(201).json(result.rows[0]);
    }
    catch (error) { // Changed 'err' to 'error: unknown'
    ...
    try {
        ...
        if (!finalCustomerId) {
            ...
            if (existingCustomerResult.rows.length > 0) {
                ...
                const newCustomerResult = await client.query(`INSERT INTO public.customers (name, total_invoiced) VALUES ($1, 0.00) RETURNING id`, [customer_name.trim()]);
                finalCustomerId = newCustomerResult.rows[0].id;
            }
        }
        const invoiceResult = await client.query(`INSERT INTO public.invoices (invoice_number, customer_id, invoice_date, due_date, total_amount, status, currency, notes)
             VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING id`, [invoice_number, finalCustomerId, invoice_date, due_date, total_amount, status || 'Draft', currency || 'ZAR', notes || null]);
        ...
        for (const item of line_items) {
            ...
            await client.query(`INSERT INTO public.invoice_line_items (invoice_id, product_service_id, description, quantity, unit_price, line_total, tax_rate)
                 VALUES ($1, $2, $3, $4, $5, $6, $7)`, [invoiceId, item.product_service_id || null, item.description, item.quantity, item.unit_price, item.line_total, item.tax_rate || 0.00]);
        }
        await client.query('COMMIT');
...
app.put('/api/invoices/:id', async (req, res) => {
    ...
    try {
        ...
        const updateInvoiceResult = await client.query(`UPDATE public.invoices
             SET
               invoice_number = $1,
               customer_id = $2,
               invoice_date = $3,
             ...
             WHERE id = $9 RETURNING id`, [invoice_number, finalCustomerId, invoice_date, due_date, total_amount, status, currency || 'ZAR', notes || null, id]);
        ...
        for (const item of line_items) {
            ...
            await client.query(`INSERT INTO public.invoice_line_items (invoice_id, product_service_id, description, quantity, unit_price, line_total, tax_rate)
                 VALUES ($1, $2, $3, $4, $5, $6, $7)`, [id, item.product_service_id || null, item.description, item.quantity, item.unit_price, item.line_total, item.tax_rate || 0.00]);
        }
        await client.query('COMMIT');
...
app.post('/api/invoices/:id/payment', async (req, res) => {
    ...
    try {
        await client.query('BEGIN');
        const transactionResult = await pool.query(`INSERT INTO public.transactions (type, amount, description, date, category, account_id)
             VALUES ($1, $2, $3, $4, $5, $6) RETURNING id`, ['income', amount_paid, transaction_description || `Payment for Invoice ${id}`, payment_date, transaction_category || 'Trading Income', account_id]);
        const transactionId = transactionResult.rows[0].id;
        await client.query(`INSERT INTO public.invoice_payments (invoice_id, transaction_id, amount_paid, payment_date, notes)
             VALUES ($1, $2, $3, $4, $5)`, [id, transactionId, amount_paid, payment_date, notes || null]);
        await client.query('COMMIT');
...
app.post('/api/quotations', async (req, res) => {
    ...
    try {
        ...
        const quotationResult = await client.query(`INSERT INTO public.quotations (quotation_number, customer_id, quotation_date, expiry_date, total_amount, status, currency, notes)
             VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING id`, [quotation_number, finalCustomerId, quotation_date, expiry_date || null, total_amount, status || 'Draft', currency || 'ZAR', notes || null]);
        const quotationId = quotationResult.rows[0].id;
        for (const item of line_items) {
            if (!item.description || item.quantity == null || item.unit_price == null || item.line_total == null) {
                throw new Error('Missing required line item fields');
            }
            await client.query(`INSERT INTO public.quotation_line_items (quotation_id, product_service_id, description, quantity, unit_price, line_total, tax_rate)
                 VALUES ($1, $2, $3, $4, $5, $6, $7)`, [quotationId, item.product_service_id || null, item.description, item.quantity, item.unit_price, item.line_total, item.tax_rate || 0.00]);
        }
        await client.query('COMMIT');
...
app.put('/api/quotations/:id', async (req, res) => {
    ...
    try {
        ...
        const updateQuotationResult = await client.query(`UPDATE public.quotations
             SET
               quotation_number = $1,
               customer_id = $2,
               quotation_date = $3,
               ...
               notes = $8,
               updated_at = CURRENT_TIMESTAMP
             WHERE id = $9 RETURNING id`, [quotation_number, finalCustomerId, quotation_date, expiry_date || null, total_amount, status, currency || 'ZAR', notes || null, id]);
        ...
        for (const item of line_items) {
            ...
            await client.query(`INSERT INTO public.quotation_line_items (quotation_id, product_service_id, description, quantity, unit_price, line_total, tax_rate)
                 VALUES ($1, $2, $3, $4, $5, $6, $7)`, [id, item.product_service_id || null, item.description, item.quantity, item.unit_price, item.line_total, item.tax_rate || 0.00] // Use 'id' here
            );
        }
        await client.query('COMMIT');
    ...
    try {
        ...
        if (!finalVendorId) {
            ...
            if (existingVendorResult.rows.length > 0) {
                ...
                const newVendorResult = await pool.query(`INSERT INTO public.vendors (name) VALUES ($1) RETURNING id`, // Assuming vendors table has 'name'
                [vendor_name.trim()]);
                finalVendorId = newVendorResult.rows[0].id;
            }
        }
        const purchaseResult = await pool.query(`INSERT INTO public.purchases (po_number, vendor_id, order_date, delivery_date, total_amount, status, currency, notes)
             VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING id`, [po_number || null, finalVendorId, order_date, delivery_date || null, total_amount, status || 'Draft', currency || 'ZAR', notes || null]);
        const purchaseId = purchaseResult.rows[0].id;
        for (const item of line_items) {
            if (!item.description || item.quantity == null || item.unit_cost == null || item.line_total == null) {
                throw new Error('Missing required line item fields');
            }
            await pool.query(`INSERT INTO public.purchase_line_items (purchase_id, product_service_id, description, quantity, unit_cost, line_total, tax_rate)
                 VALUES ($1, $2, $3, $4, $5, $6, $7)`, [purchaseId, item.product_service_id || null, item.description, item.quantity, item.unit_cost, item.line_total, item.tax_rate || 0.00]);
        }
        await client.query('COMMIT');
    ...
    try {
        ...
        if (!finalVendorId) {
            ...
            if (existingVendorResult.rows.length > 0) {
                ...
                const newVendorResult = await pool.query(`INSERT INTO public.vendors (name) VALUES ($1) RETURNING id`, [vendor_name.trim()]);
                finalVendorId = newVendorResult.rows[0].id;
            }
        }
        const updatePurchaseResult = await pool.query(`UPDATE public.purchases
             SET
               po_number = $1,
               vendor_id = $2,
               order_date = $3,
               ...
               notes = $8,
               updated_at = CURRENT_TIMESTAMP
             WHERE id = $9 RETURNING id`, [po_number || null, finalVendorId, order_date, delivery_date || null, total_amount, status, currency || 'ZAR', notes || null, id]);
        ...
        for (const item of line_items) {
            ...
            await client.query(`INSERT INTO public.purchase_line_items (purchase_id, product_service_id, description, quantity, unit_cost, line_total, tax_rate)
                 VALUES ($1, $2, $3, $4, $5, $6, $7)`, [id, item.product_service_id || null, item.description, item.quantity, item.unit_cost, item.line_total, item.tax_rate || 0.00]);
        }
        await client.query('COMMIT');
...
app.post('/api/purchases/:id/payment', async (req, res) => {
    ...
    try {
        ...
        const transactionResult = await pool.query(`INSERT INTO public.transactions (type, amount, description, date, category, account_id)
             VALUES ($1, $2, $3, $4, $5, $6) RETURNING id`, ['expense', amount_paid, transaction_description || `Payment for Purchase ${id}`, payment_date, transaction_category || 'Business Expenses', account_id]);
        const transactionId = transactionResult.rows[0].id;
        // 2. Create a purchase payment entry
        await client.query(`INSERT INTO public.purchase_payments (purchase_id, transaction_id, amount_paid, payment_date, notes)
             VALUES ($1, $2, $3, $4, $5)`, [id, transactionId, amount_paid, payment_date, notes || null]);
...
app.post('/employees', async (req, res) => {
    const { name, position, email, idNumber, phone, startDate, paymentType, baseSalary, hourlyRate, bankDetails // Object: { accountHolder, bankName, accountNumber, branchCode }
     } = req.body;
    // Basic validation
    ...
    try {
        ...
        const employeeResult = await pool.query(`INSERT INTO employees (name, position, email, id_number, phone, start_date, payment_type, base_salary, hourly_rate)
             VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING id`, [name, position || null, email, idNumber, phone || null, startDate, paymentType, baseSalary || null, hourlyRate || null]);
        ...
        if (bankDetails && bankDetails.accountHolder && bankDetails.bankName && bankDetails.accountNumber) {
            await client.query(`INSERT INTO bank_details (employee_id, account_holder, bank_name, account_number, branch_code)
                 VALUES ($1, $2, $3, $4, $5)`, [employeeId, bankDetails.accountHolder, bankDetails.bankName, bankDetails.accountNumber, bankDetails.branchCode || null]);
        }
        await client.query('COMMIT');
...
app.put('/employees/:id', async (req, res) => {
    const { id } = req.params;
    const { name, position, email, idNumber, phone, startDate, paymentType, baseSalary, hourlyRate, hoursWorked, /* Added hoursWorked */ bankDetails // Object: { accountHolder, bankName, accountNumber, branchCode }
     } = req.body;
    const client = await pool.connect();
    try {
        await client.query('BEGIN');
        ...
        let empParamIndex = 1;
        if (name !== undefined) {
            employeeUpdates.push(`name = $${empParamIndex++}`);
            employeeValues.push(name);
        }
        if (position !== undefined) {
            employeeUpdates.push(`position = $${empParamIndex++}`);
            employeeValues.push(position || null);
        }
        if (email !== undefined) {
            employeeUpdates.push(`email = $${empParamIndex++}`);
            employeeValues.push(email);
        }
        if (idNumber !== undefined) {
            employeeUpdates.push(`id_number = $${empParamIndex++}`);
            employeeValues.push(idNumber);
        }
        if (phone !== undefined) {
            employeeUpdates.push(`phone = $${empParamIndex++}`);
            employeeValues.push(phone || null);
        }
        if (startDate !== undefined) {
            employeeUpdates.push(`start_date = $${empParamIndex++}`);
            employeeValues.push(startDate);
        }
        if (paymentType !== undefined) {
            employeeUpdates.push(`payment_type = $${empParamIndex++}`);
            employeeValues.push(paymentType);
        }
        if (baseSalary !== undefined) {
            employeeUpdates.push(`base_salary = $${empParamIndex++}`);
            employeeValues.push(baseSalary || null);
        }
        if (hourlyRate !== undefined) {
            employeeUpdates.push(`hourly_rate = $${empParamIndex++}`);
            employeeValues.push(hourlyRate || null);
        }
        ...
        if (employeeUpdates.length > 1) { // More than just updated_at
            const employeeUpdateResult = await pool.query(`UPDATE employees
                 SET ${employeeUpdates.join(', ')}
                 WHERE id = $${empParamIndex} RETURNING id`, employeeValues);
        ...
        if (bankDetails) {
            ...
            const existingBankDetails = await client.query(`SELECT id FROM bank_details WHERE employee_id = $1`, [id]);
            if (existingBankDetails.rows.length > 0) {
                // Update existing bank details
                await client.query(`UPDATE bank_details
                     SET
                        account_holder = $1,
                        bank_name = $2,
                        account_number = $3,
                        branch_code = $4,
                        updated_at = CURRENT_TIMESTAMP
                     WHERE employee_id = $5`, [bankDetails.accountHolder, bankDetails.bankName, bankDetails.accountNumber, bankDetails.branchCode || null, id]);
            }
                ...
                await client.query(`INSERT INTO bank_details (employee_id, account_holder, bank_name, account_number, branch_code)
                     VALUES ($1, $2, $3, $4, $5)`, [id, bankDetails.accountHolder, bankDetails.bankName, bankDetails.accountNumber, bankDetails.branchCode || null]);
            }
        }
        else {
...
app.get('/employees/:employeeId/time-entries', async (req, res) => {
    ...
    try {
        const result = await pool.query(`SELECT id, employee_id, entry_date as date, hours_worked, notes as description, status, created_at, updated_at
             FROM time_entries
             WHERE employee_id = $1
             ORDER BY entry_date DESC`, [employeeId]);
        res.json(result.rows);
    }
    catch (error) { // Changed 'err' to 'error: unknown'
...
app.post('/employees/:employeeId/time-entries', async (req, res) => {
    ...
    try {
        const result = await pool.query(`INSERT INTO time_entries (employee_id, entry_date, hours_worked, notes, status)
...
app.put('/time-entries/:id', async (req, res) => {
    ...
    try {
        const result = await pool.query(`UPDATE time_entries
             SET ${updates.join(', ')}
...
app.get('/api/suppliers/:id', async (req, res) => {
    const { id } = req.params;
    try {
        const { rows } = await pool.query('SELECT id, name, email, phone, address, vat_number, total_purchased FROM public.suppliers WHERE id = $1', [id]);
        if (rows.length === 0) {
...
app.post('/api/suppliers', async (req, res) => {
    ...
    try {
        const result = await pool.query(`INSERT INTO public.suppliers (name, email, phone, address, vat_number)
             VALUES ($1, $2, $3, $4, $5) RETURNING id, name, email, phone, address, vat_number, total_purchased`, [name, email || null, phone || null, address || null, vatNumber || null]);
...
app.put('/api/suppliers/:id', async (req, res) => {
    ...
    try {
        const result = await pool.query(`UPDATE public.suppliers
             SET name = $1, email = $2, phone = $3, address = $4, vat_number = $5, updated_at = CURRENT_TIMESTAMP
             WHERE id = $6 RETURNING id, name, email, phone, address, vat_number, total_purchased`, [name, email || null, phone || null, address || null, vatNumber || null, id]);
        if (result.rows.length === 0) {
            return res.status(404).json({ error: 'Supplier not found.' });
        }
...
app.delete('/api/suppliers/:id', async (req, res) => {
    const { id } = req.params;
    try {
        const { rowCount } = await pool.query('DELETE FROM public.suppliers WHERE id = $1', [id]);
        if (rowCount === 0) {
            return res.status(404).json({ error: 'Supplier not found.' });
        }
        res.status(204).send(); // No Content for successful deletion
...
const getTaxRateIdFromVatRate = async (rate) => {
    if (rate === undefined || rate === null) {
        return null;
    }
    try {
        const { rows } = await pool.query('SELECT tax_rate_id FROM public.tax_rates WHERE rate = $1', [rate]);
        if (rows.length > 0) {
...
app.post('/api/products', async (req, res) => {
    const { name, description, price, costPrice, sku, isService = false, stock = 0, vatRate, category, unit } = req.body;
    // Basic validation
    ...
    try {
        const result = await pool.query(`INSERT INTO public.products_services (
        ...
        if (newProductDb.tax_rate_id) {
            const { rows: taxRows } = await pool.query('SELECT rate FROM public.tax_rates WHERE tax_rate_id = $1', [newProductDb.tax_rate_id]);
            if (taxRows.length > 0) {
...
app.put('/api/products/:id', async (req, res) => {
    const { id } = req.params;
    const { name, description, price, costPrice, sku, isService, stock, vatRate, category, unit } = req.body;
    ...
    if (isService !== undefined) {
        updates.push(`is_service = $${paramIndex++}`);
        values.push(isService);
    }
    if (stock !== undefined) {
    ...
    if (vatRate !== undefined) {
        ...
        updates.push(`tax_rate_id = $${paramIndex++}`);
    ...
    try {
        ...
        if (updatedProductDb.tax_rate_id) {
            const { rows: taxRows } = await pool.query('SELECT rate FROM public.tax_rates WHERE tax_rate_id = $1', [updatedProductDb.tax_rate_id]);
            if (taxRows.length > 0) {
...
app.delete('/api/products/:id', async (req, res) => {
    const { id } = req.params;
    try {
        const { rowCount } = await pool.query('DELETE FROM public.products_services WHERE id = $1', [id]);
        if (rowCount === 0) {
            return res.status(404).json({ error: 'Product not found.' });
        }
        res.status(204).send(); // No Content for successful deletion
...
app.get('/api/stats/clients', async (req, res) => {
    try {
        ...
        const currentResult = await pool.query('SELECT COUNT(id) AS count FROM public.customers WHERE created_at >= $1 AND created_at <= $2', [currentStart, currentEnd]);
        const previousResult = await pool.query('SELECT COUNT(id) AS count FROM public.customers WHERE created_at >= $1 AND created_at < $2', [previousStart, previousEnd]);
...
app.get('/api/stats/quotes', async (req, res) => {
    try {
        const { currentStart, currentEnd, previousStart, previousEnd } = getCurrentAndPreviousDateRanges();
        const currentResult = await pool.query('SELECT COUNT(id) AS count FROM public.quotations WHERE created_at >= $1 AND created_at <= $2', [currentStart, currentEnd]);
        const previousResult = await pool.query('SELECT COUNT(id) AS count FROM public.quotations WHERE created_at >= $1 AND created_at < $2', [previousStart, previousEnd]);
        const currentCount = parseInt(currentResult.rows[0].count, 10);
        const previousCount = parseInt(previousResult.rows[0].count, 10);
        const { changePercentage, changeType } = calculateChange(currentCount, previousCount);
        res.json({
...
app.get('/api/stats/invoices', async (req, res) => {
    try {
        const { currentStart, currentEnd, previousStart, previousEnd } = getCurrentAndPreviousDateRanges();
        const currentResult = await pool.query('SELECT COUNT(id) AS count FROM public.invoices WHERE created_at >= $1 AND created_at <= $2', [currentStart, currentEnd]);
        const previousResult = await pool.query('SELECT COUNT(id) AS count FROM public.invoices WHERE created_at >= $1 AND created_at < $2', [previousStart, previousEnd]);
        const currentCount = parseInt(currentResult.rows[0].count, 10);
        const previousCount = parseInt(previousResult.rows[0].count, 10);
        const { changePercentage, changeType } = calculateChange(currentCount, previousCount);
        res.json({
...
app.get('/api/stats/invoice-value', async (req, res) => {
    try {
        const { currentStart, currentEnd, previousStart, previousEnd } = getCurrentAndPreviousDateRanges();
        const currentResult = await pool.query('SELECT COALESCE(SUM(total_amount), 0) AS value FROM public.invoices WHERE created_at >= $1 AND created_at <= $2', [currentStart, currentEnd]);
        const previousResult = await pool.query('SELECT COALESCE(SUM(total_amount), 0) AS value FROM public.invoices WHERE created_at >= $1 AND created_at < $2', [previousStart, previousEnd]);
...
app.post('/documents', upload.single('file'), async (req, res) => {
    try {
        ...
        const result = await pool.query(`INSERT INTO documents (user_id, name, type, description, file_url, file_mime_type, file_size_bytes)
       VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *`, [user_id, name, type, description, fileUrl, mimeType, fileSize]);
        res.status(201).json(result.rows[0]);
    }
    catch (error) { // Changed 'err' to 'error: unknown'
...
app.post('/api/tasks', async (req, res) => {
    ...
    try {
        const result = await pool.query(`INSERT INTO public.tasks (user_id, title, description, status, priority, due_date, progress_percentage, project_id, created_at)
...
app.get('/api/tasks', async (req, res) => {
    const dummyUserId = 'frontend-user-123';
    try {
        const result = await pool.query(`SELECT t.id, t.title, t.description, t.status, t.priority, t.due_date, t.progress_percentage, t.created_at, t.updated_at,
...
app.put('/api/tasks/:id', async (req, res) => {
    ...
    const status = getStatusFromPercentage(progress_percentage);
    try {
        const result = await pool.query(`UPDATE public.tasks
...
app.delete('/api/tasks/:id', async (req, res) => {
    ...
    const dummyUserId = 'frontend-user-123';
    try {
        const result = await pool.query(`DELETE FROM public.tasks WHERE id = $1 AND user_id = $2 RETURNING id`, [id, dummyUserId]);
        if (result.rows.length === 0) {
            return res.status(404).json({ error: 'Task not found or unauthorized.' });
        }
...
app.post('/api/projects', async (req, res) => {
    ...
    try {
        const result = await pool.query(`INSERT INTO public.projects (name, description, deadline, status, assignee, progress_percentage, created_at, updated_at)
...
app.get('/api/projects', async (req, res) => {
    try {
        const result = await pool.query(`SELECT id, name, description, deadline, status, assignee, progress_percentage, created_at, updated_at
...
app.put('/api/projects/:id', async (req, res) => {
    ...
    try {
        const result = await pool.query(`UPDATE public.projects
...
app.delete('/api/projects/:id', async (req, res) => {
    const { id } = req.params;
    try {
        const result = await pool.query(`DELETE FROM public.projects WHERE id = $1 RETURNING id`, [id]);
        if (result.rows.length === 0) {
            return res.status(404).json({ error: 'Project not found.' });
        }
        res.status(204).send(); // No Content
    ...
    try {
        ...
        const formatCurrencyForPdf = (amount) => {
            if (amount === null || amount === undefined)
                return '-'; // Handle null/undefined balances
            if (amount === 0)
                return '-';
        ...
        const drawDocumentHeader = (doc, companyName, documentTitle, dateString, disclaimerText = null) => {
            ...
            doc.moveDown();
            if (disclaimerText) {
                doc.fontSize(8).fillColor('red').text(disclaimerText, { align: 'center', width: doc.page.width - 100, continued: false });
        ...
        switch (documentType) {
            case 'income-statement': {
                const incomeStatementStartDate = startDate;
                const incomeStatementEndDate = endDate;
          ...
          `, [incomeStatementStartDate, incomeStatementEndDate]);
                ...
                let otherIncome = 0;
                const detailedIncome = {}; // To store income by category for display
          ...
          `, [incomeStatementStartDate, incomeStatementEndDate]);
                ...
                drawDocumentHeader(doc, companyName, 'INCOME STATEMENT', `FOR THE PERIOD ENDED ${new Date(incomeStatementEndDate).toLocaleDateString('en-GB', { day: '2-digit', month: 'long', year: 'numeric' })}`);
                // Table Headers
            ...
            case 'balance-sheet': {
                const balanceSheetEndDate = endDate;
        ...
        `, [balanceSheetEndDate]);
          ...
          `, [balanceSheetEndDate] // Calculate all profit/loss up to balance sheet date
                ...
                drawDocumentHeader(doc, companyName, 'BALANCE SHEET', `AS OF ${new Date(balanceSheetEndDate).toLocaleDateString('en-GB', { day: '2-digit', month: 'long', year: 'numeric' })}`);
                // ASSETS SECTION
                ...
                doc.lineWidth(1).strokeColor('#a0aec0').moveTo(col1X, doc.y).lineTo(col2X + columnWidth, doc.y).stroke();
                doc.moveDown();
                doc.fontSize(8).fillColor('#4a5568').text(`Statement Period: ${new Date(startDate).toLocaleDateString('en-GB')} to ${new Date(endDate).toLocaleDateString('en-GB')}`, { align: 'center' });
                doc.fillColor('black');
                doc.moveDown();
                break;
            }
            case 'trial-balance': {
                const trialBalanceStartDate = startDate;
                const trialBalanceEndDate = endDate;
          ...
          `, [trialBalanceEndDate]);
                ...
                drawDocumentHeader(doc, companyName, 'TRIAL BALANCE', `AS OF ${new Date(trialBalanceEndDate).toLocaleDateString('en-GB')}`, 'Disclaimer: This trial balance is based on the provided bank statement and assumptions about transaction categorization that are LLM based utilising relevant accounting libraries.');
                // Table Headers
                doc.font('Helvetica-Bold');
                doc.fillColor('#e2e8f0').rect(col1X, doc.y, doc.page.width - 100, 20).fill(); // Background for header
                ...
                doc.text(formatCurrencyForPdf(totalDebit), debitX, doc.y + 5, { width: columnWidth, align: 'right' });
                doc.text(formatCurrencyForPdf(totalCredit), creditX, doc.y + 5, { width: columnWidth, align: 'right' });
                doc.moveDown();
                doc.fillColor('black'); // Reset text color
                doc.font('Helvetica');
            ...
            case 'cash-flow-statement': {
                const classify = (row) => {
                    ...
                    if (['equipment', 'property', 'asset', 'vehicle'].some(k => cat.includes(k)))
                        return 'investing';
                    if (['loan', 'members loan', 'shareholders loan', 'credit facility'].some(k => cat.includes(k)))
                        return 'financing';
                    return 'operating';
                };
                const cashFlows = {
                ...
                const rowsResult = await pool.query(`SELECT type, category, amount FROM transactions WHERE date >= $1 AND date <= $2 AND (type = 'income' OR type = 'expense');`, [startDate, endDate]);
                const rows = rowsResult.rows;
                rows.forEach((row) => {
                    const section = classify(row);
                    const amount = row.type === 'income' ? parseFloat(row.amount) : -parseFloat(row.amount);
                ...
                drawDocumentHeader(doc, companyName, 'CASH FLOW STATEMENT', `FOR THE PERIOD ENDED ${new Date(endDate.toString()).toLocaleDateString('en-GB', { day: '2-digit', month: 'long', year: 'numeric' })}`);
